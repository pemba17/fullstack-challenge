...
const fetchData = async () => {
  ...
  apiResponse.value.users.forEach(async (user, index) => {
    await axios
      .get(
...
const { weatherDetails, load } = getWeather();
...
<template>
  ...
  <div v-if="apiResponse">
    <div class="container mx-auto p-8 m-10">
        ...
        >
          ...
          <tbody>
            ...
            >
              ...
              <td class="px-6 py-4">{{ user.longitude }}</td>
              <td class="px-6 py-4">{{ user.latitude }}</td>
              <td class="px-6 py-4">{{ user.weather }}</td>
              <td class="px-6 py-4">
                <a
  ...
  <Modal size="lg" v-if="isShowModal" @close="closeModal">
    <template #header>
      <div class="flex items-center text-lg">Weather Details</div>
    </template>
    <template #body>
      <weather-modal :weatherDetails="weatherDetails" />
    </template>
  </Modal>
</template>
